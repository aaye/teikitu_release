...
/*  »Project«   Teikitu Gaming System (TgS) (∂)
    »File«      TgS (WIN) Kernel - Constants.h
...
/* =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-= */
#if !defined(TGS_WIN_KERNEL_CONSTANTS_H)
#define TGS_WIN_KERNEL_CONSTANTS_H
#pragma once
...
enum { KTgKN_MAX_WINDOWS                    =     9 };  /**< Maximum number of job threads used by the kernel */
