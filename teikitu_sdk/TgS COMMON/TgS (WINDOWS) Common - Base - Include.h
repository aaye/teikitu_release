...
/*  »Project«   Teikitu Gaming System (TgS) (∂)
    »File«      TgS (WINDOWS) Common - Base - Include.h
...
/* =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#if !defined(TGS_WINDOWS_COMMON_BASE_INCLUDE_H)
#define TGS_WINDOWS_COMMON_BASE_INCLUDE_H
#pragma once
...
TgCLANG_WARN_DISABLE_PUSH(nonportable-system-include-path)
...
TgCLANG_WARN_DISABLE_POP(nonportable-system-include-path)
