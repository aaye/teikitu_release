...
TgINLINE TgRESULT tgCM_UT__CMD_BUF__Init( STg2_UT__CMD_BUF_PCU psCMD_BUF, TgUINT_E32_C nbyMax )
{
    ...
    psCMD_BUF->m_pBuffer = TgCOMMIT_VIRTUAL( psCMD_BUF->m_pBuffer, nbyMax );
    if (nullptr == psCMD_BUF->m_pBuffer)
    {
    ...
    atomic_store( &psCMD_BUF->m_uiREQ_NBY, 0 );
    atomic_store( &psCMD_BUF->m_nuiCompleted, 0 );
...
TgINLINE TgRSIZE tgCM_UT__CMD_BUF__Query_Used_Size( STg2_UT__CMD_BUF_PCU psCMD_BUF )
{
    ...
    sOrig.m_uiREQ_NBY = atomic_load( &psCMD_BUF->m_uiREQ_NBY );
...
TgINLINE TgRESULT tgCM_UT__CMD_BUF__Queue_Command( STg2_UT__CMD_BUF_PCU psCMD_BUF, TgFCN_CMD_FUNCTION pfnCommand, TgRSIZE_C nbyParam, TgVOID_CPC pParam )
{
    ...
    while(!atomic_compare_exchange_weak( &psCMD_BUF->m_uiREQ_NBY, &sOrig.m_uiREQ_NBY, sNew.m_uiREQ_NBY ));
    ...
    atomic_fetch_add( &psCMD_BUF->m_nuiCompleted, 1 );
    return (KTgS_OK);
}
...
TgINLINE TgVOID tgCM_UT__CMD_BUF__Execute( STg2_UT__CMD_BUF_PCU psCMD_BUF )
{
    ...
    sOrig.m_uiREQ_NBY = atomic_load( &psCMD_BUF->m_uiREQ_NBY );
    while(!atomic_compare_exchange_weak( &psCMD_BUF->m_nuiCompleted, &sOrig.m_nuiRequested, sOrig.m_nuiRequested ))
    {
        tgTR_Pause();
        sOrig.m_uiREQ_NBY = atomic_load( &psCMD_BUF->m_uiREQ_NBY );
    }
