...
/** @brief Initialize a management object with ARG1 number of reserved bytes.
...
tgCM_UT__CMD_BUF__Init(
    STg2_UT__CMD_BUF_PCU NONULL OUT0, TgUINT_E32_C ARG1 );
...
tgCM_UT__CMD_BUF__Query_Used_Size(
    STg2_UT__CMD_BUF_PCU NONULL ARG );
...
/** @brief Acquire the next index for the producer (writer)
    @param [in] ARG0 Pointer to a management object for the look aside ring buffer
...
tgCM_UT__CMD_BUF__Queue_Command(
    STg2_UT__CMD_BUF_PCU NONULL ARG0, TgFCN_CMD_FUNCTION ARG1, TgRSIZE_C ARG2, TgVOID_CPC ARG3 );
