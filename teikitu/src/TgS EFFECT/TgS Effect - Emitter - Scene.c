...
{
    STg2_FX__Emitter_P                  psEmitter;
    TgUINT_F32                          uiEmitter_Frame_Mesh_Set__LOD_Index;
    ...
    TgUINT_F32                          uiEmitter_Frame_Mesh__Flag;
    STg2_FX__Emitter__Particle_P        psParticle;
    ...
    uiIndex_In_SE_Context = atomic_fetch_add( &psSE_Context->m_xnuiFrame_Mesh, psEmitter->m_sExtend.m_nuiParticle );
    if (uiIndex_In_SE_Context + psEmitter->m_sExtend.m_nuiParticle >= psSE_Context->m_nuiFrame_Mesh_Max)
    {
        atomic_fetch_sub( &psSE_Context->m_xnuiFrame_Mesh, psEmitter->m_sExtend.m_nuiParticle );
        return;
    }
    ...
    while (uiIndex_In_SE_Context < uiMax_Index_In_SE_Context)
    {
        ...
        tgMM_Copy( pRN_Cxt_Mesh->m_auiLight, sizeof( pRN_Cxt_Mesh->m_auiLight ), auiEmitter_Frame_Mesh_Set__Light_List, KTgRN_CXT_MAX_LIGHT_PER_INSTANCE * sizeof( TgUINT_F32  ) );
