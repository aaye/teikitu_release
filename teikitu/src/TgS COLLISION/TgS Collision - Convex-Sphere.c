...
/*  »Project«   Teikitu Gaming System (TgS) (∂)
    »File«      TgS Collision - Convex-Sphere.c
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT FCN_VO(tgCO_SP_Penetrate_CX)(
    VEC_OBJ_T(STg2_CO_Packet,PC) psPacket, VEC_OBJ_T(TgSPHERE,CPC) psSP0, VEC_OBJ_T(TgSTRI,CPC) apsST, VEC_OBJ_T(TgBOXAA,CPC) apsBA, TgRSIZE_C nuiTri )
{
    TgRSIZE_C                           nuiContact = psPacket->m_nuiContact;
    VEC_OBJ_T(STg2_CO_Packet)           sTriPacket;
    VEC_OBJ_T(STg2_CO_Contact)          asContact[8];
    TgRSIZE                             uiIndex;
    VEC_OBJ_T(TgBOXAA)                  sBA_SP0;
    ...
    FCN_VO(tgGM_SP_BA)( &sBA_SP0, psSP0 );

    for (uiIndex = 0; uiIndex < nuiTri; ++uiIndex)
    {
        if (!FCN_VO(tgCO_BA_Test_BA)( apsBA + uiIndex, &sBA_SP0 ))
        {
            continue;
        };

        if (FCN_VO(tgCO_ST_Penetrate_SP)( &sTriPacket, apsST + uiIndex, psSP0) < 0)
        {
            continue;
        };
