...
/*  »Project«   Teikitu Gaming System (TgS) (∂)
    »File«      TgS Collision - Convex-Box.c
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT FCN_VO(tgCO_BX_Penetrate_CX)(
    VEC_OBJ_T(STg2_CO_Packet,PC) psPacket, VEC_OBJ_T(TgBOX,CPC) psBX0, VEC_OBJ_T(TgSTRI,CPC) apsST, VEC_OBJ_T(TgBOXAA,CPC) apsBA, TgRSIZE_C nuiTri )
{
    TgRSIZE_C                           nuiContact = psPacket->m_nuiContact;
    VEC_OBJ_T(STg2_CO_Packet)           sTriPacket;
    VEC_OBJ_T(STg2_CO_Contact)          asContact[8];
    TgRSIZE                             uiIndex;
    VEC_OBJ_T(TgBOXAA)                  sBA_BX0;
    VEC_OBJ_T(STg2_CO_Contact,P)        psContact;
    TgRSIZE                             uiTriIndex;

    TgPARAM_CHECK( nullptr != psPacket );
    TgPARAM_CHECK( nullptr != psBX0 );
    ...
    FCN_VO(tgGM_BX_BA)( &sBA_BX0, psBX0 );

    for (uiIndex = 0; uiIndex < nuiTri; ++uiIndex)
    {
        if (!FCN_VO(tgCO_BA_Test_BA)( apsBA + uiIndex, &sBA_BX0 ))
        {
            continue;
        };

        if (FCN_VO(tgCO_ST_Penetrate_BX)( &sTriPacket, apsST + uiIndex, psBX0 ) < 0)
        {
            continue;
        };
