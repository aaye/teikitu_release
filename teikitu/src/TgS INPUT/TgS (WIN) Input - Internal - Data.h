...
/*  »Project«   Teikitu Gaming System (TgS) (∂)
    »File«      TgS (WIN) Input - Internal - Data.h
...
/* =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
#if !defined(TGS_WIN_INPUT_INTERNAL_DATA_H)
#define TGS_WIN_INPUT_INTERNAL_DATA_H
#pragma once
...
TgEXTN HRESULT                              (__stdcall *g_pfnDirectInput8Create)( HINSTANCE, TgUINT_F32, TgVOID_CP, TgVOID_PP, TgVOID_P );
...
TgEXTN TgUINT_F32                           (__stdcall *g_pfnXInputSetState)( TgUINT_F32, TgVOID_P );
TgEXTN TgUINT_F32                           (__stdcall *g_pfnXInputGetState)( TgUINT_F32, TgVOID_P );
TgEXTN TgUINT_F32                           (__stdcall *g_pfnXInputGetCapabilities)( TgUINT_F32, TgUINT_F32, TgVOID_P );

/* (WIN) KERNEL, DLL: user32 */
TgEXTN TgBOOL                               (__stdcall *g_pfnScreenToClient)( HWND,TgVOID_P );

/* (WIN) KERNEL, DLL: api-ms-win-core-localization-l1-2-1 */
