...
T(TgKN_GPU_,_INST_ID) T(tgKN_GPU_,_Inst__Init)( UTg2_KN_GPU_CMD_C uCMD, T(TgKN_GPU_,_ID_C) sID_RS, TgCHAR_U8_CPCU puszName )
{
    ...
    sID_RS_INST.m_uiK = atomic_fetch_add( &psLibRSI->m_xnuiInit, 1 ) & ((1 << KTgKN_GPU_RSI_BIT__SALT) - 1);
    sID_RS_INST.m_uiLoad = 1;
    sID_RS_INST.m_uiFree = 0;
    sID_RS_INST.m_uiRef = 1;

    TgERROR(sID_RS_INST.m_uiIndex < psLibRSI->m_nuiRSI);
    atomic_store( psLibRSI->m_pxsID_Inst_Ref_Singleton + sID_RS_INST.m_uiIndex, sID_RS_INST );
    ...
    if (TgFAILED( iResult ))
    {
        ...
        atomic_store( psLibRSI->m_pxsID_Inst_Ref_Singleton + sID_RS_INST.m_uiIndex, T(KTgKN_GPU_,_INST_ID__INVALID) );
...
/*# TgS_STAT__KERNEL || TgS_DEBUG__KERNEL */
    ...
    sOrig = atomic_load( psLibRSI->m_pxsID_Inst_Ref_Singleton + sID_RS_INST.m_uiIndex );
    do
    {
        sNew.m_uiKI = sOrig.m_uiKI;
        sNew.m_uiLoad = 0;
    }
    while(!atomic_compare_exchange_weak( psLibRSI->m_pxsID_Inst_Ref_Singleton + sID_RS_INST.m_uiIndex, &sOrig, sNew ));
...
T(TgKN_GPU_,_INST_ID) T(tgKN_GPU_, _Inst__Release)( T(TgKN_GPU_,_INST_ID_C) sID_RS_INST )
{
    ...
    do
    {
        ...
        /*# TgS_STAT__KERNEL || TgS_DEBUG__KERNEL */
            ...
            atomic_store( psLibRSI->m_pxsID_Inst_Ref_Singleton + sOrig.m_uiIndex, T(KTgKN_GPU_,_INST_ID__INVALID) );
            ...
            while(!atomic_compare_exchange_weak( psLibRSI->m_pxsID_Inst_Ref_Singleton + sID_RS_INST.m_uiIndex, &sOrig, T(KTgKN_GPU_,_INST_ID__INVALID) ));
...
T(TgKN_GPU_,_ID) T(tgKN_GPU_,__IncRef)( T(TgKN_GPU_,_ID_C) sID_RS )
{
    ...
    sOrig = atomic_load( T(g_axsKN_Lib_,_Ref_Singleton) + sID_RS.m_uiIndex );
    do
    {
    ...
    while(!atomic_compare_exchange_weak( T(g_axsKN_Lib_,_Ref_Singleton) + sID_RS.m_uiIndex, &sOrig, sNew ));
...
T(TgKN_GPU_,_ID) T(tgKN_GPU_,__Release)( T(TgKN_GPU_,_ID_C) sID_RS )
{
    ...
    do
    {
        ...
        /*# TgS_STAT__KERNEL || TgS_DEBUG__KERNEL */
            ...
            while(!atomic_compare_exchange_weak( T(g_axsKN_Lib_,_Ref_Singleton) + sID_RS.m_uiIndex, &sOrig, T(KTgKN_GPU_,_ID__INVALID) ));
...
static T(TgKN_GPU_,_ID) T(tgKN_GPU_,__Load_Internal)( TgKN_FILE_ID_C tiFile, TgRSIZE_C uiFile_Offset, T(STg2_KN_GPU_,_DESC_CPC) psDESC, ETgKN_GPU_ALLOCATOR_C enAllocator )
{
    ...
    sID.m_uiK = atomic_fetch_add( &T(g_xnuiKN_Lib_,_Init), 1 ) & ((1 << KTgKN_GPU_RS_BIT__SALT) - 1);
    sID.m_uiLoad = 1;
    sID.m_uiFree = 0;
    sID.m_uiRef = 1;

    TgERROR_INDEX(sID.m_uiIndex, T(g_asKN_Lib_,_Data));
    atomic_store( T(g_axsKN_Lib_,_Ref_Singleton) + sID.m_uiIndex, sID );
    ...
    if (TgFAILED(iResult))
    {
        atomic_store( T(g_axsKN_Lib_,_Ref_Singleton) + sID.m_uiIndex, T(KTgKN_GPU_,_ID__INVALID) );
        tgCM_UT_LF__ST__Push( &T(g_sKN_Lib_,).m_sStack, sLibData.psNode );
    ...
    /* 4. Mark the resource as loading complete. */
    sOrig = atomic_load( T(g_axsKN_Lib_,_Ref_Singleton) + sID.m_uiIndex );
    do
    {
        sNew.m_uiKI = sOrig.m_uiKI;
        sNew.m_uiLoad = 0;
    }
    while(!atomic_compare_exchange_weak( T(g_axsKN_Lib_,_Ref_Singleton) + sID.m_uiIndex, &sOrig, sNew ));
