...
TgVOID T(tgKN_GPU_,_LIB__Init)( TgVOID )
{
    ...
    atomic_store( &T(g_xnuiKN_Lib_,_Init), 1001);
...
TgVOID T(tgKN_GPU_,_LIB__Stop)( TgVOID )
{
    ...
    for (uiIndex = 0; uiIndex < T(KTgKN_GPU_MAX_,); ++uiIndex)
    {
        ...
        while(!atomic_compare_exchange_weak( T(g_axsKN_Lib_,_Ref_Singleton) + uiIndex, &sOrig, T(KTgKN_GPU_,_ID__INVALID) ))
        {
        #if TgS_DEBUG__KERNEL
            ...
            sNew = sOrig;
            sNew.m_uiRef = 1;
            while(!atomic_compare_exchange_weak( T(g_axsKN_Lib_,_Ref_Singleton) + uiIndex, &sOrig, sNew ))
            {
                sNew = sOrig;
                sNew.m_uiRef = 1;
            }
...
TgVOID T(tgKN_GPU_,_LIB__Free)( TgVOID )
{
...
#if TgCOMPILE__MEM_FREE_CLEAR
    {
        ...
        atomic_store( &T(g_xnuiKN_Lib_,_Init), 0 );
...
TgVOID T(tgKN_GPU_,_Inst_LIB__Init)( TgKN_GPU_CXT_EXEC_ID_C tiCXT_DATA )
{
    ...
    atomic_store( &g_asKN_GPU_CXT_EXEC[tiCXT_DATA.m_uiI].T(m_s,_Inst_LIB_DESC).m_xnuiInit, 1001 );
}
...
TgVOID T(tgKN_GPU_,_Inst_LIB__Free)( TgKN_GPU_CXT_EXEC_ID_C tiCXT_DATA )
{
    ...
    atomic_store( &g_asKN_GPU_CXT_EXEC[tiCXT_DATA.m_uiI].T(m_s,_Inst_LIB_DESC).m_xnuiInit, 0 );
