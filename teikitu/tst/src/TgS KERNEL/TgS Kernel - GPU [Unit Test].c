...
static TgSINT_F64 __stdcall
tgUT_KN_GPU_Base_Window_Update(
    HWND hWnd, TgUINT_F32 uMessage, TgUINT_F64 wParam, TgSINT_F64 lParam );
...
TEST_METHOD( UNIT_TEST__TEST__KN_GPU_Render_Init_Windows )
{
#if defined(TgBUILD_FEATURE__GRAPHICS) && !defined(TgFEATURE_GRAPHICS__REF) && defined(TgBUILD_OS__WIN)
    ...
    {
        TgSINT_F32_C                        iWindow_X0 = sWin_Config.m_iWindow_X0 - (TgSINT_E32)sWin_Config.m_uiRender_Target_W - tgKN_Query_Window_PadX() - 25;
        TgSINT_F32_C                        iWindow_Y0 = sWin_Config.m_iWindow_Y0 - (TgSINT_E32)sWin_Config.m_uiRender_Target_H - tgKN_Query_Window_PadY() - 25;
...
/*# defined(TgBUILD_FEATURE__GRAPHICS) && !defined(TgFEATURE_GRAPHICS__REF) && defined(TgBUILD_OS__WIN) */
#endif
    TEST_END_METHOD( KTgS_OK );
}
...
static TgRESULT UNIT_TEST__TEST__KN_GPU_Render_Simple_Job( STg2_Job_PC psJob )
{
    ...
    atomic_fetch_add( &s_xuiRender_Counter, 1 );
    return (KTgS_OK);
}
/*# defined(TgBUILD_FEATURE__GRAPHICS) && !defined(TgFEATURE_GRAPHICS__REF) */
#endif
...
TEST_METHOD( UNIT_TEST__TEST__KN_GPU_Render_Simple )
{
    ...
    atomic_store( &s_xuiRender_Counter, 0 );
    ...
    for (s_uiFrame_Counter = 0; !bAll_Windows_Closed; ++s_uiFrame_Counter)
    {
        ...
        nuiSwap = 0;
        uiFrame_Counter_Start = atomic_load( &s_xuiRender_Counter );
        for (uiWindow = 0; uiWindow < s_nuiLimit_Windows; ++uiWindow)
        {
        ...
        while( uiFrame_Counter_Start + nuiSwap != atomic_load( &s_xuiRender_Counter ))
        {
            tgTR_Yield();
        };
...
/* ---- tgUT_KN_GPU_Base_Window_Update ------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgSINT_F64 __stdcall tgUT_KN_GPU_Base_Window_Update( HWND hWnd, TgUINT_F32 uMessage, TgUINT_F64 wParam, TgSINT_F64 lParam )
{
