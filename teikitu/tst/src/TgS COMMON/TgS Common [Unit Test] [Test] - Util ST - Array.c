...
TEST_METHOD( UNIT_TEST__TEST__CM_UT_ST__AR__Init )
{
    TgUINT_F32                          uiDataB[] = { 2,1,0,4,5,6,3 };
    STg2_UT_ST__AR                      sKB;
...
TEST_METHOD( UNIT_TEST__TEST__CM_UT_ST__AR__Assign_ElementN )
{
    TgUINT_F32                          uiElement5;
    TgUINT_F32                          uiElement7;
    TgUINT_F32                          uiReserve;
    TgUINT_F32                          uiSize;
    TgUINT_F32                          uiLength;
    TgUINT_F32                          uiCheck;
    ...
    for (uiReserve = 0; uiReserve <= 28; uiReserve = 4 + (uiReserve * 2))
    {
        for (uiSize = 0; uiSize <= uiReserve; uiSize = 4 + (uiSize * 2))
        {
            for (uiLength = 0; uiLength <= 32; ++uiLength)
            {
                ...
                {
                    Test__Expect_EQ(uiElement5, *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
                };
                ...
                {
                    Test__Expect_EQ(uiElement7, *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
                };
        ...
        for (uiSize = 0; uiSize <= uiReserve; uiSize = 4 + (uiSize * 2))
        {
            for (uiBegin = 0; uiBegin <= TgARRAY_COUNT( uiDataB ); ++uiBegin)
            {
                for (uiEnd = 0; uiBegin + uiEnd <= TgARRAY_COUNT( uiDataB ); ++uiEnd)
                {
                    ...
                    for (uiCheck = 0; uiCheck < uiLength; ++uiCheck)
                    {
                        Test__Expect_EQ(uiDataB[uiBegin + uiCheck], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
                    };
                    uiCheck = 0 == uiSize ? 0u : tgCM_MAX_U32( uiReserve, uiSize );
                    Test__Expect_EQ(0 == uiLength ? 0u : tgCM_MAX_U32( uiLength, uiCheck ), tgCM_UT_ST__AR__Capacity( &sKB ));
                    tgCM_UT_ST__AR__Free( &sKB );
...
TEST_METHOD( UNIT_TEST__TEST__CM_UT_ST__AR__Push_Back )
{
    ...
    for (uiReserve = 0; uiReserve <= 28; uiReserve = 4 + (uiReserve * 2))
    {
        for (uiSize = 0; uiSize <= uiReserve; uiSize = 4 + (uiSize * 2))
        {
            ...
            Test__Expect_EQ(uiElement7, *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiSize ));
    ...
    for (uiReserve = 0; uiReserve <= 28; uiReserve = 4 + (uiReserve * 2))
    {
        for (uiSize = 0; uiSize <= uiReserve; uiSize = 4 + (uiSize * 2))
        {
            for (uiIndex = 0; uiIndex <= uiSize; ++uiIndex)
            {
                ...
                {
                    Test__Expect_EQ(uiCheck == uiIndex ? uiElement7 : uiElement5, *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
                };
                Test__Expect_EQ(tgCM_MAX_U32( 0 == uiSize ? 0 : uiReserve, uiSize + 1 ), tgCM_UT_ST__AR__Capacity( &sKB ));
                tgCM_UT_ST__AR__Free( &sKB );
            };
    ...
    for (uiReserve = 0; uiReserve <= 28; uiReserve = 4 + (uiReserve * 2))
    {
        for (uiSize = 0; uiSize <= uiReserve; uiSize = 4 + (uiSize * 2))
        {
            for (uiIndex = 0; uiIndex <= uiSize; ++uiIndex)
            {
                ...
                {
                    Test__Expect_EQ(uiDataB[uiCheck - uiIndex], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
                };
        ...
        for (uiIndex = 0; uiIndex <= 7; ++uiIndex)
        {
            ...
            for (; uiCheck < uiIndex; ++uiCheck)
            {
                Test__Expect_EQ(uiDataB[uiCheck], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
            };
            for (; uiCheck < uiIndex + 7; ++uiCheck)
            {
                Test__Expect_EQ(uiDataB[uiCheck - uiIndex], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
            ...
            {
                Test__Expect_EQ(uiDataB[uiCheck - 7], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
            };
            ...
            for (uiIndex = 0; uiIndex <= uiSize; ++uiIndex)
            {
                for (uiBegin = 0; uiBegin <= 7; ++uiBegin)
                {
                    for (uiEnd = 0; uiBegin + uiEnd <= 7; ++uiEnd)
                    {
                        ...
                        {
                            Test__Expect_EQ(uiDataB[uiCheck - uiIndex + uiBegin], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
                        };
                        ...
                        {
                            Test__Expect_EQ(uiDataB[uiCheck - (7 - (uiBegin + uiEnd))], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
                        };
...
TEST_METHOD( UNIT_TEST__TEST__CM_UT_ST__AR__Erase_Element )
{
    ...
    for (uiIndex = 0; uiIndex < 7; ++uiIndex)
    {
        ...
        {
            Test__Expect_EQ(uiDataB[uiCheck + (uiCheck >= uiIndex ? 1 : 0)], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
        };
        Test__Expect_EQ(7, tgCM_UT_ST__AR__Capacity( &sKB ));
    }
...
TEST_METHOD( UNIT_TEST__TEST__CM_UT_ST__AR__Erase_Range )
{
    ...
    for (uiBegin = 0; uiBegin <= 7; ++uiBegin)
    {
        for (uiEnd = 0; uiBegin + uiEnd <= 7; ++uiEnd)
        {
            ...
            {
                Test__Expect_EQ(uiDataB[uiCheck], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiIndex ));
            };
...
TEST_METHOD( UNIT_TEST__TEST__CM_UT_ST__AR__Get_Index )
{
    ...
    for (uiIndex = 0; uiIndex < 7; ++uiIndex)
    {
        Test__Expect_EQ(uiDataB[uiIndex], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiIndex ));
    };
...
TEST_METHOD( UNIT_TEST__TEST__CM_UT_ST__AR__Get_Front )
{
    ...
    Test__Expect_EQ(uiDataB[0], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Front( &sKB ));
...
TEST_METHOD( UNIT_TEST__TEST__CM_UT_ST__AR__Get_Back )
{
    ...
    Test__Expect_EQ(uiDataB[6], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Back( &sKB ));
...
TEST_METHOD( UNIT_TEST__TEST__CM_UT_ST__AR__Capacity )
{
    ...
    Test__Expect_EQ(uiDataB[0], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, 0 ));
    Test__Expect_EQ(uiDataB[0], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, 1 ));
    Test__Expect_EQ(uiDataB[0], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, 2 ));
...
TEST_METHOD( UNIT_TEST__TEST__CM_UT_ST__AR__Resize )
{
    ...
    for (uiSize = 0; uiSize <= 9; ++uiSize)
    {
        ...
        {
            Test__Expect_EQ(uiDataB[0], *(TgUINT_F32_P)tgCM_UT_ST__AR__Get_Index( &sKB, uiCheck ));
        };
