...
# --------------------------------------------------------------------------------------------------------------
    ...
    [ValidateSet("C17_X64", "INTEL_X64", "C17_ARM")]
    [string]$CPU,
...
if ($IDE -match "MSVC17")
{
    ...
    If ($CPU -match "C17_X64" -or $CPU -match "INTEL_X64")
    {
...
{
    $CMAKE_BUILD_COMMON_CMD += "-DMK_COMPILE_PLATFORM_TEXT__WIDE=0"
}
...
if ($CICD_BUILD)
{
    git tag $CMAKE_BUILD_PATH -a -f -m "CircleCI Build"
}
