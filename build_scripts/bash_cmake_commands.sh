...
if [ ! ${log_status} -eq 0 ]; then
    CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} --log-level=STATUS"
fi

if [ ! ${fresh} -eq 0 ]; then
    CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} --fresh"
fi

case $ide in
  UNIX)
    CMAKE_BUILD_COMMON_CMD=${CMAKE_BUILD_COMMON_CMD}' -G "Unix Makefiles"'
    ;;
  NINJA)
    CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -G Ninja"
    ;;
  XCODE)
    CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -G Xcode"
    ;;
  *)
...
if [ ${compiler^^} = CLANG ]; then
  CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DCMAKE_C_COMPILER=/usr/bin/clang"
  CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DCMAKE_CXX_COMPILER=/usr/bin/clang++"
fi
CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DCMAKE_BUILD_TYPE=${build_type^^}"
CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_BUILD__TARGET_HARDWARE=${cpu^^}"
CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_BUILD__THREAD=${thread^^}"
CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_BUILD__OS=${os^^}"
CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_BUILD__DEVICE=${device^^}"
CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_FEATURE__GRAPHICS=${gpu^^}"
CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_FEATURE__AUDIO=${audio^^}"

if [ ! ${cicd_build} -eq 0 ]; then
{
    CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_BUILD__AUTOMATION=CICD_BUILD"
    CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_BUILD__LOG_TO_FILE=0"
}
else
{
    CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_BUILD__AUTOMATION=NONE"
    CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_BUILD__LOG_TO_FILE=1"
}
fi

if [ ! ${cicd_build} -eq 0 ] || [ ! ${teikitu} -eq 0 ]; then
{
    CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_BUILD__LIST=TEIKITU"
}
else
{
    CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DMK_BUILD__LIST=ALL"
}
fi
...
# CMAKE COMMAND LINE
if [ ! ${verbose} -eq 0 ]; then
  printf '\033[36;1m%s\033[0m\n' "CMAKE COMMAND: cmake -S .. -B \"../build/${CMAKE_BUILD_PATH,,}\" ${CMAKE_BUILD_COMMON_CMD}"
  cmake "${CMAKE_BUILD_COMMON_CMD}" -S .. -B ../build/${CMAKE_BUILD_PATH,,}
else
  cmake "${CMAKE_BUILD_COMMON_CMD}" -S .. -B ../build/${CMAKE_BUILD_PATH,,} 1> /dev/null
fi

# BUILD COMMAND
if [ ! ${verbose} -eq 0 ]; then
  printf '\033[36;1m%s\033[0m\n' "CMAKE COMMAND: cmake --build \"../build/${CMAKE_BUILD_PATH}\" --config ${build_type^^}"
  cmake --build ../build/${CMAKE_BUILD_PATH,,} --config ${build_type^^}
else
  cmake --build ../build/${CMAKE_BUILD_PATH,,} --config ${build_type^^} 1> /dev/null
fi

# BUILD TAG
if [ ! ${cicd_build} -eq 0 ]; then
  git tag -d build__hwin_${CMAKE_BUILD_PATH,,}
  git tag build__hwin_${CMAKE_BUILD_PATH,,} -m "Success" --no-edit -s
fi
