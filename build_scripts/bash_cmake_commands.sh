...
if [ ${compiler^^} = CLANG ]; then
  CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DCMAKE_C_COMPILER=/usr/bin/clang-16"
  CMAKE_BUILD_COMMON_CMD="${CMAKE_BUILD_COMMON_CMD} -DCMAKE_CXX_COMPILER=/usr/bin/clang++-16"
fi
...
if [ ! ${verbose} -eq 0 ]; then
  ...
  cmake -S .. -B ../build/${CMAKE_BUILD_PATH,,} "${CMAKE_BUILD_COMMON_CMD}"
else
  cmake -S .. -B ../build/${CMAKE_BUILD_PATH,,} "${CMAKE_BUILD_COMMON_CMD}" 1> /dev/null
fi

# BUILD COMMAND
if [ ! ${verbose} -eq 0 ]; then
  printf '\033[36;1m%s\033[0m\n' "CMAKE COMMAND: cmake --build \"../build/${CMAKE_BUILD_PATH}\" --config ${build_type^^}"
  cmake --build ../build/${CMAKE_BUILD_PATH,,} --config ${build_type^^} -- -k -j
else
  cmake --build ../build/${CMAKE_BUILD_PATH,,} --config ${build_type^^} -- -k -j 1> /dev/null
fi
