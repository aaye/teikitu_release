...
if [ ${build} -ne 0 ]; then
  if [ ${verbose} -ne 0 ]; then
    ...
    if [ ${parallel} -ne 0 ]; then
        cmake --build "./${CMAKE_BUILD_PATH}" --config ${build_type} --parallel ${parallel}
    else
        cmake --build "./${CMAKE_BUILD_PATH}" --config ${build_type}
    fi
  else
    if [ ${parallel} -ne 0 ]; then
        cmake --build "./${CMAKE_BUILD_PATH}" --config ${build_type} --parallel ${parallel}  1> /dev/nu
    else
        cmake --build "./${CMAKE_BUILD_PATH}" --config ${build_type}  1> /dev/nu
    fi
  fi
fi
