...
TIFFOpen(const char* name, const char* mode)
{
	...
	switch(m) {
		case _O_RDONLY:			dwMode = OPEN_EXISTING; break;
		case _O_RDWR:			dwMode = OPEN_ALWAYS;   break;
		case _O_RDWR|O_CREAT:		dwMode = OPEN_ALWAYS;   break;
		case _O_RDWR|O_TRUNC:		dwMode = CREATE_ALWAYS; break;
		case _O_RDWR|O_CREAT|O_TRUNC:	dwMode = CREATE_ALWAYS; break;
		default:			return ((TIFF*)0);
	}
        
	fd = (thandle_t)CreateFileA(name,
		(m == _O_RDONLY)?GENERIC_READ:(GENERIC_READ | GENERIC_WRITE),
		FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, dwMode,
		(m == _O_RDONLY)?FILE_ATTRIBUTE_READONLY:FILE_ATTRIBUTE_NORMAL,
		NULL);
