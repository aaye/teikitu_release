...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgBOOL tgFX__Update__Trail_Create_Command( STg2_FX_Instance_CP psInst, TgFX_DATA_INST_ID_C tiData, TgATTRIBUTE_MAYBE_UNUSED TgVOID_CP pData1 )
{
    ...
    {
        tgFX__Update__Set_Render_Data_From_Material_Hash( nullptr, &psFX->m_tiMaterial, &psFX->m_enVertex, psFX->m_psFile_Data->m_uiHash_Material );
...
/*# defined(TgBUILD_FEATURE__GRAPHICS_VFX) */
    ...
    psFX->m_sExtend.m_avPos_N2W[0] = psInst->m_vPos_N2W;
    psFX->m_sExtend.m_vScaleFactor = tgMH_SPX_F32_04_1( psInst->m_vScale );
    psFX->m_sExtend.m_afU[0] = 0.0F;
...
TgVOID tgFX__Update__Trail_Update( TgVEC_F32_04_1 vdT, TgFLOAT32_C fDT, ETgFX_UPDATE_C enUpdate )
{
    ...
    while (KTgID__INVALID_VALUE != tiActive.m_uiKI)
    {
        ...
        TgUINT_E32                          uiIndex;
        TgUINT_E32                          uiRead_Index;
        TgUINT_E32                          uiWrite_Index;
        ...
        if (ETgFX_EFFECT_STATE__UPDATE__ACTIVE_WAITING_TO_DIE == g_aenFX__Trail__Shared__State[tiActive.m_uiI])
        {
            ...
            {
                // Every update, make sure to consume at least one point of the trail.
        ...
        uiWrite_Index = (psTrail->m_sExtend.m_uiPoint_Head + KTgFX_MAX_POINTS_IN_TRAIL - 1) % KTgFX_MAX_POINTS_IN_TRAIL;
        while (1)
        {
            ...
            vNext_Pos_W = tgMH_ADD_F32_04_1( psTrail->m_sExtend.m_avPos_N2W[uiRead_Index], vRolling_Sum_Of_Pos_W_Delta );
            vDelta = tgMH_SUB_F32_04_1( vNext_Pos_W, psTrail->m_sExtend.m_avPos_N2W[uiIndex] );
            uDelta_Length.m_vF32_04_1 = tgMH_LEN_F32_04_1( vDelta );
            ...
            vNew_Delta = tgMH_MUL_F32_04_1( vDelta, vFollow_Fraction );
            psTrail->m_sExtend.m_avPos_N2W[uiWrite_Index] = tgMH_ADD_F32_04_1( psTrail->m_sExtend.m_avPos_N2W[uiIndex], vNew_Delta );
        ...
        {
            psTrail->m_sExtend.m_afU[uiIndex] = vRolling_Sum_Of_Length[uiIndex].m_aData[0] * fInv_Max_Trail_Length;
...
TgVOID tgFX__Update__Trail_Process_Update_Command( ETgFX_COMMAND_C enCommand, TgVOID_CP pData, TgFX_DATA_INST_ID_C tiData )
{
    ...
    switch (enCommand)
    {
        ...
        TgATTRIBUTE_FALLTHROUGH case ETgFX_COMMAND__CLIENT_TO_UPDATE__POSITION_UPDATE_FROM_PARENT:
            TgERROR( g_aenFX__Trail__Shared__State[tiData.m_uiI] >= ETgFX_EFFECT_STATE__UPDATE__ACTIVE );
            tgFX__Update__Trail_Add_Point( tiData.m_uiI, sUnion.psCommand_1_F32_04->m_vF32_04 );
            break;
        case ETgFX_COMMAND__CLIENT_TO_UPDATE__ROTATION_SET: /* Fall Through - Rotation is not conceptually valid */
        TgATTRIBUTE_FALLTHROUGH case ETgFX_COMMAND__CLIENT_TO_UPDATE__ROTATION_UPDATE_FROM_PARENT: /* Fall Through - Rotation is not conceptually valid */
        TgATTRIBUTE_FALLTHROUGH case ETgFX_COMMAND__CLIENT_TO_UPDATE__VISIBLE: /* Fall Through - Intentionally Blank */
            break;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgVOID tgFX__Update__Trail_Add_Point( TgRSIZE_C uiIndex, TgVEC_F32_04_1 vNewPoint )
{
    STg2_FX__Trail_P                    psTrail;
    STg2_FX_Trail__File_Data_CP         psFile_Data;
    TgUINT_E32                          nuiPoints;
    ...
    // Check to see if we have non-zero number of points
    if (psTrail->m_sExtend.m_uiPoint_Head != psTrail->m_sExtend.m_uiPoint_Tail)
    {
        // Check to see if this new point is within error of the most recently added point.
        if (tgMH_PRX3_CMP_EQ_F32_04_1( psTrail->m_sExtend.m_avPos_N2W[psTrail->m_sExtend.m_uiPoint_Head], vNewPoint ))
        {
            return;
        };
    };

    // Constrain the list of points to be less than the maximum points set in the configuration.
    ...
    // Increment the head of the list, and assign the new point.
    psTrail->m_sExtend.m_uiPoint_Head = (psTrail->m_sExtend.m_uiPoint_Head + 1) % KTgFX_MAX_POINTS_IN_TRAIL;
    psTrail->m_sExtend.m_avPos_N2W[psTrail->m_sExtend.m_uiPoint_Head] = vNewPoint;
}
