...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgBOOL tgFX__Update__Emitter_Create_Command( TgATTRIBUTE_MAYBE_UNUSED STg2_FX_Instance_CP psInst, TgFX_DATA_INST_ID_C tiData, TgVOID_CP pData1 )
{
...
static TgBOOL tgFX__Update__Emitter_Reserve_Particle_Buffer( STg2_FX__Emitter_P psEmitter )
{
    ...
    uiMemory_Current = TgSTD_ATOMIC(load)( &g_xuiFX__Emitter__Memory_Current );
    do
    {
    ...
    while (!TgSTD_ATOMIC(compare_exchange_weak)( &g_xuiFX__Emitter__Memory_Current, &uiMemory_Current, uiMemory_New ));
    ...
    if (nullptr == TgCOMMIT_VIRTUAL( psEmitter->m_sExtend.m_psParticle_Memory, 0, uiMin_Commit * uiPage_Size ))
    {
        uiMemory_Current = TgSTD_ATOMIC(load)( &g_xuiFX__Emitter__Memory_Current );
        do
        {
