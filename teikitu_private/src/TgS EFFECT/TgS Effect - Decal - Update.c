...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgBOOL tgFX__Update__Decal_Create_Command( STg2_FX_Instance_CP psInst, TgFX_DATA_INST_ID_C tiData, TgATTRIBUTE_MAYBE_UNUSED TgVOID_CP pData1 )
{
...
TgVOID tgFX__Update__Decal_Update( TgVEC_F32_04_1 vdT, TgFLOAT32_C fDT, ETgFX_UPDATE_C enUpdate )
{
    ...
    while (KTgID__INVALID_VALUE != tiActive.m_uiKI)
    {
        ...
        if (ETgFX_EFFECT_STATE__UPDATE__ACTIVE_FIRST == g_aenFX__Decal__Shared__State[tiActive.m_uiI])
        {
            ...
            tgCO_REQ_Init_Collect_F32_04( &sRequest.m_sCollision_Request, ETgPM_SP, ETgCO_PURPOSE__DECAL );
            tgCO_REQ_Set_Include_Filter_F32_04( &sRequest.m_sCollision_Request, ETgCO_FILTER__RENDER );
            tgGM_SP_Init_F32_04( &sRequest.m_sCollision_Request.m_uPM.m_sSP, psDecal->m_sExtend.m_vPos_N2W, uSize.m_vF32_04_1 );
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID tgFX__Update__Decal_Process_Update_Command( ETgFX_COMMAND_C enCommand, TgATTRIBUTE_MAYBE_UNUSED TgVOID_CP pData, TgFX_DATA_INST_ID_C tiData )
{
#if defined(TgBUILD_FEATURE__GRAPHICS_VFX)
...
static TgBOOL tgFX__Update__Decal_Reserve_Cache_Buffer( STg2_FX__Decal_P psDecal )
{
    ...
    uiMemory_Current = TgSTD_ATOMIC(load)( &g_xuiFX__Decal__Memory_Current );
    do
    {
    ...
    while (!TgSTD_ATOMIC(compare_exchange_weak)( &g_xuiFX__Decal__Memory_Current, &uiMemory_Current, uiMemory_New ));
...
static TgBOOL tgFX__Update__Decal_Commit_Cache_Buffer( STg2_FX__Decal_P psDecal, TgRSIZE_C nuiVert )
{
    ...
    if (nullptr == TgCOMMIT_VIRTUAL( psDecal->m_sExtend.m_psVert, 0, uiMin_Commit ))
    {
        uiMemory_Current = TgSTD_ATOMIC(load)( &g_xuiFX__Decal__Memory_Current );
        do
        {
...
static TgVOID tgFX__Update__Decal_Update__Write_Cache( STg2_FX__Decal_P psDecal, STg2_CO_Request_F32_04_CP psCO_Request )
{
    ...
    sClipList.pui = (TgUINT_E08_P)TgMALLOC_POOL(sizeof( STg2_CO_Clip_List_F32_04 ) + 9 * sizeof( TgVEC_F32_04_1 ));
    TgCRITICAL_MSGF( nullptr != sClipList.pui, STD_MSG_LITERAL_1, STD_MSG_POST, u8"Failed to allocate memory." );
    ...
    for (uiCollect = 0; uiCollect < psCO_Request->m_nuiCollect_Last; ++uiCollect)
    {
        ...
        for (enPlane = ETgFRUSTUM_PLANE__NEAR; enPlane < ETgFRUSTUM_PLANE__MAX; enPlane = (ETgFRUSTUM_PLANE)((TgSINT_E32)enPlane + 1))
        {
