...
/* =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-==-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
    ...
    TgMSVC_WARN_DISABLE_POP(6235) /* Analysis - (<non-zero constant> || <expression>) is always a non-zero constant. */
    TgMSVC_WARN_DISABLE_POP(6240) /* Analysis - (<expression> && <non-zero constant>) always evaluates to the result of <expression>. */
