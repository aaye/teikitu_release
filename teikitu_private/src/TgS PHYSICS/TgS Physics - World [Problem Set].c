...
tgPH_Update_World__Build_Sets__Validate( 
    TgPH_WORLD_ID_C ARG0, TgUINT_E64_P TgANALYSIS_NO_NULL ARG1, TgUINT_E64_P TgANALYSIS_NO_NULL ARG2 );
...
/* SYNC: READS Bodies, Constraints - Responsible for creating the self-contained (independent) Solver Sets                                                                         */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgPH_Update_World__Build_Sets( TgPH_WORLD_ID_C tiWorld )
{
    ...
    TgRSIZE_C                           nuiConstraint_World = TgSTD_ATOMIC(load_explicit)( g_axnuiPH_Constraint_Total__Used + tiWorld.m_uiI, TgSTD_MEMORY_ORDER(relaxed) );
    ...
    tgPH_Update_World__Init_Solver_Set( psSolver_Set, nuiBody_World, nuiConstraint_World, g_aaxnuiPH_Constraint_Total__Used_By_Type[tiWorld.m_uiI] );
    #pragma endregion
    ...
    for (uiIndex_Body = 0; uiIndex_Body < nuiBody_World; ++uiIndex_Body)
    {
        ...
        if (0 != (auiVisited_Body[uiIndex_Body >> 6] & (1ULL << (uiIndex_Body & 63ULL))))
            continue;
        ...
        auiVisited_Body[uiIndex_Body >> 6] |= 1ULL << (uiIndex_Body & 63ULL);
        nuiBody_Stack = 0;
        ...
        while(1)
        {
            ...
            tiConstraint.m_uiKI = psBY->m_tiCT.m_uiKI;
            while (KTgID__INVALID_VALUE != tiConstraint.m_uiKI) TgATTRIBUTE_LIKELY
            {
                TgDIAG_MSGF( nuiConstraint_World > 0, STD_MSG_LITERAL_1, STD_MSG_POST, u8"Logic Failure: a valid constraint id in a world with no constraints." );
                psCT = g_aasPH_Constraint[tiWorld.m_uiI] + tiConstraint.m_uiI;

                TgDIAG(psCT->m_tiConstraint.m_uiKI == tiConstraint.m_uiKI);
                TgDIAG_MSGF( psCT == g_aapsPH_Constraint_Used[tiWorld.m_uiI][psCT->m_uiUsed_Index], STD_MSG_LITERAL_1, STD_MSG_POST,
                                u8"Logic Failure: constraint used index does not match to the same constraint." );

                /* Test the visited bit for this constraint. */
                while (0 == (auiVisited_Constraint[psCT->m_uiUsed_Index >> 6] & (1ULL << (psCT->m_uiUsed_Index & 63ULL))))
                {
                    #pragma region Add a constraint to the set

                    /* Mark the constraint as visited. */
                    auiVisited_Constraint[psCT->m_uiUsed_Index >> 6] |= (1ULL << (psCT->m_uiUsed_Index & 63ULL));
                    ...
                    if (KTgID__INVALID_VALUE != tiBody.m_uiKI)
                    {
                        ...
                        if (0 == (auiVisited_Body[psBYX->m_uiUsed_Index >> 6] & (1ULL << (psBYX->m_uiUsed_Index & 63ULL))))
                        {
                            TgPARAM_CHECK(g_aapsPH_Body_Used[tiWorld.m_uiI][psBYX->m_uiUsed_Index] == psBYX);
                            auiVisited_Body[psBYX->m_uiUsed_Index >> 6] |= (1ULL << (psBYX->m_uiUsed_Index & 63ULL));
                            apsBody_Stack[nuiBody_Stack] = g_aapsPH_Body_Used[tiWorld.m_uiI][psBYX->m_uiUsed_Index];
                            ++nuiBody_Stack;
                        };
                        #pragma endregion
...
static TgVOID tgPH_Update_World__Build_Sets__Validate( TgPH_WORLD_ID_C tiWorld, TgUINT_E64_P auiVisited_Body, TgUINT_E64_P auiVisited_Constraint )
{
    ...
    for (uiIndex_Body = 0; uiIndex_Body < nuiBody_World; ++uiIndex_Body)
    {
        STg2_PH_Body_PC                     psBY = g_aapsPH_Body_Used[tiWorld.m_uiI][uiIndex_Body];
        ...
        if (psBY->m_bEnabled)
        {
            ...
            if (0 == (auiVisited_Body[uiIndex_Body >> 6] & (1ULL << (uiIndex_Body & 63ULL))))
            {
                TgDIAG_MSGF( false, STD_MSG_LITERAL_1, STD_MSG_POST, u8"Enabled body was not visited." );
            }
        }
        else
        {
            ...
            if (0 != (auiVisited_Body[uiIndex_Body >> 6] & (1ULL << (uiIndex_Body & 63ULL))))
            {
                TgDIAG_MSGF( false, STD_MSG_LITERAL_1, STD_MSG_POST, u8"Disabled body was visited." );
            }
        }
    }
    ...
    {
        STg2_PH_Constraint_PC               psCT = g_aapsPH_Constraint_Used[tiWorld.m_uiI][uiIndex_Constraint];
        ...
        {
            /* Test the visited bit for this constraint. */
            if (0 != (auiVisited_Constraint[uiIndex_Constraint >> 6] & (1ULL << (uiIndex_Constraint & 63ULL))))
            {
                TgDIAG_MSGF( false, STD_MSG_LITERAL_1, STD_MSG_POST, u8"Disabled constraint was visited." );
            }
        }
    }
}
