...
/*# defined(TgBUILD_FEATURE__SERVER) */
    ...
    atomic_store( &g_xnuiPH_History, 0 );
...
TgVOID tgPH_Module_Free( TgVOID )
{
    ...
    /* Free the worlds. */
    for (iWorld = (TgSINT_F32)ETgPH_WORLD__BEGIN; iWorld < (TgSINT_F32)ETgPH_WORLD__END; ++iWorld)
    {
        tgCM_UT_LF__RW__Free( &g_asPH_World_Data_Lock[iWorld].m_sLock );
    };
...
/* ---- tgPH_Module_Update ------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgPH_Module_Update( TgFLOAT32_C UNUSED_PARAM fDT )
{
    TgUN_SCALAR                         uScalar;
