...
TgRESULT tgKN_GPU_EXT__SwapChain__Init( TgKN_GPU_CXT_SWAP_ID_C tiCXT_SWAP, TgBOOL_C bResize )
{
    ...
    else
    {
        ...
        TgVERIFY(SUCCEEDED(ID3D12Device10_CreateFence( psDX12_Device, TgSTD_ATOMIC(load)( &psEXT_CXT_SWAP->m_xuiFence_Value ), D3D12_FENCE_FLAG_NONE, MS_REF IID_ID3D12Fence,
                                                       (TgVOID_PP) &psEXT_CXT_SWAP->m_psDX12_Fence ) ) );
        TgSTD_ATOMIC(fetch_add)( &psEXT_CXT_SWAP->m_xuiFence_Value, 1 );
    ...
    if (bFirst_Time_Init || bResize)
    {
        ...
        uiFence_Value = TgSTD_ATOMIC(fetch_add)( &psEXT_CXT_SWAP->m_xuiFence_Value, 1 );
