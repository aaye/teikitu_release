...
ETgKN_IO_STATUS tgKN_FS_JB_Status( TgKN_FILE_JOB_ID_C tiFile_Job )
{
    ...
    return ((ETgKN_IO_STATUS)TgSTD_ATOMIC(load)( &g_asKN_File_Job[tiFile_Job.m_uiI].m_xuiStatus ));
}
...
TgKN_FILE_ID tgKN_File__Info_Init( STg2_KN_File_Open_Command_CPCU psCmd )
{
    ...
    sFile_Info.ps->m_uiMode = psCmd->m_uiMode;
    sFile_Info.ps->m_niCmd = 0;
    sFile_Info.ps->m_niExecute = 0;

    tgKN_FILE_ID_Init( &sFile_Info.ps->m_tiFile_Singleton, (TgUINT_E32)(sFile_Info.ps - g_asKN_File_Info) );
...
TgFORCEINLINE TgBOOL tgKN_FS_Open_Command_Test( STg2_KN_File_Open_Command_CPCU psCmd )
{
    ...
    if ((psCmd->m_uiMode & ETgFILE_IO_ACCESS__ACCESS_MASK) != (psCmd->m_uiMode & g_asKN_FS_Mount[psCmd->m_tiFS_Mount.m_uiI].m_uiMode))
    {
        TgERROR_MSGF( 0, STD_MSG_LITERAL_1, STD_MSG_POST, u8"Invalid File Access Mode request for Mount Point." );
        return (false);
    };

    if (ETgFILE_IO_ACCESS__CREATE == (psCmd->m_uiMode & ETgFILE_IO_ACCESS__CREATE))
    {
...
static TgBOOL tgKN_FS_Command_Test( TgKN_FILE_ID_C tiFile, ETgFILE_COMMAND_C enFile_Operation )
{
    ...
    if ((ETgFILE_COMMAND__CLOSE != enFile_Operation) && (ETgFILE_IO_ACCESS__CLOSING == (ETgFILE_IO_ACCESS__CLOSING & g_asKN_File_Info[tiFile.m_uiI].m_uiMode)))
    {
        return (false);
    };
    ...
    switch (enFile_Operation)
    {
        case ETgFILE_COMMAND__READ_ASYNC:
            if (ETgFILE_IO_ACCESS__READ != (ETgFILE_IO_ACCESS__READ & g_asKN_File_Info[tiFile.m_uiI].m_uiMode))
            {
        ...
        TgATTRIBUTE_FALLTHROUGH case ETgFILE_COMMAND__WRITE_ASYNC:
            if (ETgFILE_IO_ACCESS__WRITE != (ETgFILE_IO_ACCESS__WRITE & g_asKN_File_Info[tiFile.m_uiI].m_uiMode))
            {
        ...
        case ETgFILE_COMMAND__UNKNOWN: TgATTRIBUTE_FALLTHROUGH
        case ETgFILE_COMMAND__OPEN: TgATTRIBUTE_FALLTHROUGH
        case ETgFILE_COMMAND__CACHE: TgATTRIBUTE_FALLTHROUGH
        case ETgFILE_COMMAND__CLOSE: TgATTRIBUTE_FALLTHROUGH
...
TgFORCEINLINE STg2_KN_File_Job_P tgKN_FS_Mount_Get_Job_Command( TgVOID )
{
    ...
    tgKN_FILE_JOB_ID_Init( &sJob.ps->m_tiFile_Job_Singleton, (TgUINT_E32)(sJob.ps - g_asKN_File_Job) );
    ...
    TgSTD_ATOMIC(store)( &sJob.ps->m_xuiStatus, ETgKN_IO_STATUS__INVALID );
...
TgKN_FILE_ID tgKN_FS_JB_Open_Internal( STg2_KN_File_Open_Command_CPCU psCmd, TgBOOL_C bBlocking )
{
    ...
    if (!tgKN_FILE_ID_Is_Equal( &g_asKN_File_Info[tiFile.m_uiI].m_tiFile_Singleton, tiFile ) || ETgKN_IO_STATUS__INVALID == TgSTD_ATOMIC(load)( &psJob->m_xuiStatus) )
    {
...
static TgVOID tgKN_FS_Job_Queue( STg2_KN_File_Job_PCU psJob, TgBOOL_C bBlocking )
{
    ...
    TgSTD_ATOMIC(store)( &psJob->m_xuiStatus, ETgKN_IO_STATUS__QUEUED );
...
static TgVOID tgKN_FS_Job_Complete_Final( STg2_KN_File_System_PC psFS, STg2_KN_File_Job_P psJob )
{
    ...
    if (ETgFILE_COMMAND__CANCEL == psJob->m_enCmd)
    {
        do
        {
            ...
            if (!TgSTD_ATOMIC(compare_exchange_strong)( &g_asKN_File_Job[tiFile_Job.m_uiI].m_xuiStatus, &uiStatus_Expected, ETgKN_IO_STATUS__CANCEL ))
            {
                break;
            };
    ...
    case ETgFILE_COMMAND__OPEN: TgATTRIBUTE_FALLTHROUGH
    case ETgFILE_COMMAND__FLUSH:
        psFile_Info->m_uiMode |= ETgFILE_IO_ACCESS__OVERLAPPED;
        break;

    case ETgFILE_COMMAND__WRITE_ASYNC: TgATTRIBUTE_FALLTHROUGH
    case ETgFILE_COMMAND__READ_ASYNC:
        --psFile_Info->m_niExecute;
        --psFS->m_niExecute;
        break;
    ...
    case ETgFILE_COMMAND__CLOSE: TgATTRIBUTE_FALLTHROUGH
    case ETgFILE_COMMAND__CANCEL: TgATTRIBUTE_FALLTHROUGH
...
static TgVOID tgKN_FS_Job_Execute( STg2_KN_File_System_PC psFS )
{
    do
    {
        ...
        do
        {
            ...
            case ETgFILE_COMMAND__OPEN: TgATTRIBUTE_FALLTHROUGH
            case ETgFILE_COMMAND__FLUSH:
                psFile_Info->m_uiMode &= ~(TgUINT_E32)ETgFILE_IO_ACCESS__OVERLAPPED;
                if (0 == psFile_Info->m_niExecute)
                {
                    ...
                    if (TgSTD_ATOMIC(compare_exchange_strong)( &sJob.ps->m_xuiStatus, &uiKN_IO_Status, ETgKN_IO_STATUS__EXECUTE ))
                    {
            ...
            case ETgFILE_COMMAND__READ_ASYNC:
                ...
                if (ETgFILE_IO_ACCESS__OVERLAPPED == (psFile_Info->m_uiMode & ETgFILE_IO_ACCESS__OVERLAPPED))
                {
                    TgRSIZE                             uiKN_IO_Status;
