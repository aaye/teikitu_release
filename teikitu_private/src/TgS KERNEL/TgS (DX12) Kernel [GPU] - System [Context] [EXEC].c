...
tgKN_GPU_EXT_Command_Queue_Init(
    STg2_KN_GPU_EXT_CMD_Queue_PC TgANALYSIS_NO_NULL ARG0, ID3D12Device10_P ARG1, D3D12_COMMAND_LIST_TYPE ARG2, TgUINT_E32_C ARG3 );

static TgVOID
tgKN_GPU_EXT_Command_Queue_Free(
    STg2_KN_GPU_EXT_CMD_Queue_PC TgANALYSIS_NO_NULL ARG0 );

static TgVOID
tgKN_GPU_EXT_Context_Command_Cmd_Allocators_Init(
    STg2_KN_GPU_EXT_CXT_CMD_PC TgANALYSIS_NO_NULL ARG0 );
...
tgKN_GPU_EXT_Context_Command_Init(
    TgKN_GPU_CXT_EXEC_ID_C tiCXT_EXEC, STg2_KN_GPU_EXT_CXT_CMD_PC TgANALYSIS_NO_NULL ARG0 );
...
TgVOID tgKN_GPU_EXT__Execute__Cmd_List_Flush( TgKN_GPU_CXT_EXEC_ID_C tiCXT_EXEC, ETgKN_GPU_EXT_COMMAND_C enCmd_List )
{
    ...
    TgPARAM_CHECK( enCmd_List >= ETgKN_GPU_EXT_COMMAND_DIRECT && enCmd_List <= ETgKN_GPU_EXT_COMMAND_VIDEO_ENCODE );

    psEXT_Queue = &psEXT_CXT_EXEC->m_sCmd_Queue[enCmd_List];

    uiFence_Value = TgSTD_ATOMIC(fetch_add)( &psEXT_Queue->m_xuiFence_Value, 1 );
...
TgKN_GPU_CXT_WORK_ID tgKN_GPU_EXT__Execute__Frame_Start( TgKN_GPU_CXT_EXEC_ID_C tiCXT_EXEC )
{
    ...
    {
        STg2_KN_GPU_EXT_CXT_EXEC_PC         psEXT_CXT_EXEC = g_asKN_GPU_EXT_CXT_EXEC + tiCXT_EXEC.m_uiI;
        TgRSIZE                             uiContext_Command_Count = TgSTD_ATOMIC(fetch_add)( &psEXT_CXT_EXEC->m_xuiContext_Command_Index, 1 );
        ...
        TgSTD_ATOMIC(store)( &psEXT_CXT_CMD->m_xuiDX12_CB_Text, 0 );
        TgSTD_ATOMIC(store)( &psEXT_CXT_CMD->m_xuiDX12_CB_UI, 0 );
        TgSTD_ATOMIC(store)( &psEXT_CXT_CMD->m_xuiDX12_CB_GEOM, 0 );
        TgSTD_ATOMIC(store)( &psEXT_CXT_CMD->m_xuiDX12_VB_LINE, 0 );
...
TgVOID tgKN_GPU_EXT__CMD__Command_List_Close( UTg2_KN_GPU_CMD_C uCMD )
{
    ...
    TgPARAM_CHECK( tgKN_GPU_CXT_EXEC_ID_Is_Equal( &psCXT_EXEC->m_tiCXT_EXEC_S, psEXT_CXT_EXEC->m_sContext_Command[uiWORK].m_tiCXT_EXEC ) );
    TgPARAM_CHECK( uCMD.psEXT->m_enCmd <= ETgKN_GPU_EXT_COMMAND_VIDEO_ENCODE );
    ...
    uCMD.psEXT->m_enCmd = (ETgKN_GPU_EXT_COMMAND)(D3D12_COMMAND_LIST_TYPE_VIDEO_ENCODE + 1);
...
TgVOID tgKN_GPU_EXT__Execute__Register_Resource_For_Release( UTg2_KN_GPU_CMD_C uCMD, ID3D12Resource_P psDX12_Resource )
{
...

    {
        ...
        psEXT_Release_Resource->m_uiFence_Value = TgSTD_ATOMIC(load)( &psEXT_CXT_EXEC->m_sCmd_Queue[uCMD.psEXT->m_enCmd].m_xuiFence_Value );
                                               ...
                                               TgUINT_E32_C uiNodeMask )
{
    ...
    {
        uiFence_Value = TgSTD_ATOMIC(fetch_add)( &psCmd_Queue->m_xuiFence_Value, 1 );
