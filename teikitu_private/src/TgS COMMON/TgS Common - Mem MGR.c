...
/*# TgS_STAT__COMMON */
    ...
    sMGR.m_pfnCommit_Size = tgMM_OS__Commit_Size;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID_P tgMM_Malloc_With_Trace( ETgMM_ALLOCATOR_C enAllocator, TgRSIZE_C uiSize, TgRSIZE_C uiAlignment, TgCHAR_U8_CPC mbzF, TgUINT_E32_C uiL )
{
    TgVOID_P                            pReturn;
...
/* ---- tgMM_Commit_Size --------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgMM_Commit_Size( TgRSIZE_P pnbyResult, ETgMM_ALLOCATOR_C enAllocator, TgRSIZE_C nbyCommit )
{
    TgERROR(ETgMODULE_STATE__BOOTED == s_enMem_MGR_State);
    TgPARAM_CHECK_INDEX( enAllocator, s_asAllocator );

    if ((nullptr == pnbyResult) || (!s_asAllocator[enAllocator].m_pfnCommit_Size))
    {
        return (KTgE_FAIL);
    };

    *pnbyResult = s_asAllocator[enAllocator].m_pfnCommit_Size( nbyCommit );
...
#if TgS_STAT__COMMON
    atomic_fetch_sub( s_anuiStat_Allocations + sInfo.m_enAllocator, 1 );
    if (TgSUCCEEDED(iResult))
    {
        TgRSIZE                             uiValue;

        uiValue = sInfo.m_bReserved ? sInfo.m_uiSize : 0;
        atomic_fetch_sub( s_auiStat_Reserved + sInfo.m_enAllocator, uiValue );
        uiValue = sInfo.m_bReserved ? 0 : sInfo.m_uiSize;
        atomic_fetch_sub( s_auiStat_Allocated + sInfo.m_enAllocator, uiValue );
    };
#else
    (void)iResult;
#endif
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgVOID_P tgMM_Realloc_With_Trace( ETgMM_ALLOCATOR_C enAllocator, TgVOID_PC pMem, TgRSIZE_C uiSize, TgRSIZE_C uiAlignment, TgCHAR_U8_CPC mbzF, TgUINT_F32_C uiL )
{
    TgVOID_P                            pReturn;

    if (nullptr == pMem)
    {
        return (tgMM_Malloc_With_Trace( enAllocator, uiSize, uiAlignment, mbzF, uiL ));
    };
...
TgVOID_P tgMM_Realloc( ETgMM_ALLOCATOR_C enAllocator, TgVOID_PC pMem, TgRSIZE_C uiSize, TgRSIZE_C uiAlignment )
{
    ...
    if (pReturn != pMem)
    {
        ...
        atomic_fetch_sub( s_anuiStat_Allocations + sOld_Info.m_enAllocator, 1 );

        uiValue = sOld_Info.m_bReserved ? sOld_Info.m_uiSize : 0;
        atomic_fetch_sub( s_auiStat_Reserved + sOld_Info.m_enAllocator, uiValue );
        uiValue = sOld_Info.m_bReserved ? 0 : sOld_Info.m_uiSize;
        atomic_fetch_sub( s_auiStat_Allocated + sOld_Info.m_enAllocator, uiValue );
    }
#endif
...
TgINLINE TgVOID tgMM_Stat_Internal__Entry( TgMEM_INFO_HEADER_P psInfo )
{
    ...
    uiValue = atomic_fetch_add( s_anuiStat_Allocations + psInfo->m_enAllocator, 1 ) + 1;
    if (uiValue > (uiUpdate = atomic_load( s_anuiStat_Max_Allocations + psInfo->m_enAllocator )))
    {
        while (!atomic_compare_exchange_strong( s_anuiStat_Max_Allocations + psInfo->m_enAllocator, &uiUpdate, uiValue) && (uiValue > uiUpdate));
    };

    nbyReserved = psInfo->m_bReserved ? psInfo->m_uiSize : 0;
    nbyAllocated = psInfo->m_bReserved ? 0 : psInfo->m_uiSize;

    uiValue = atomic_fetch_add( s_auiStat_Reserved + psInfo->m_enAllocator, nbyReserved ) + nbyReserved;
    if (uiValue > (uiUpdate = atomic_load( s_auiStat_Max_Reserved + psInfo->m_enAllocator )))
    {
        while (!atomic_compare_exchange_strong( s_auiStat_Max_Reserved + psInfo->m_enAllocator, &uiUpdate, uiValue) && (uiValue > uiUpdate));
    };

    uiValue = atomic_fetch_add( s_auiStat_Allocated + psInfo->m_enAllocator, nbyAllocated ) + nbyAllocated;
    if (uiValue > (uiUpdate = atomic_load( s_auiStat_Max_Allocated + psInfo->m_enAllocator )))
    {
        while (!atomic_compare_exchange_strong( s_auiStat_Max_Allocated + psInfo->m_enAllocator, &uiUpdate, uiValue) && (uiValue > uiUpdate));
    };
}
/*# TgS_STAT__COMMON */
#endif
...
/* ---- tgMM_OS__Commit_Size ----------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRSIZE tgMM_OS__Commit_Size( TgRSIZE_C uiSize )
{
...
static TgRESULT tgMM_Pool__Malloc( TgMEM_INFO_HEADER_P psInfo, TgVOID_PP ppReturn, TgRSIZE_C uiSize, TgRSIZE_C uiAlignment )
{
    ...
    if (nullptr == uMem.psMem_Page)
    {
        ...
        {
            TgRSIZE_C                           uiBlock_Size = (1 << (uiBitPool)) + (TgUINT_F32)uiPool_LSD*(1 << (uiBitPool - 2));
...
/* ---- tgMM_MI__Init_MGR -------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgMM_MI__Init_MGR( TgVOID )
{
    return (KTgS_OK);
}
...
/* ---- tgMM_MI__Free_MGR -------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgMM_MI__Free_MGR( TgVOID )
{
    return (KTgS_OK);
}
...
/* ---- tgMM_MI__Size ------------------------------------------------------------------------------------------------------------------------------------------------------------ */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgMM_MI__Size( TgUINT_F32_P pnbyReserve, TgUINT_F32_P pnbyAllocated, TgVOID_CPC pMem )
{
    (void)pnbyReserve;
    (void)pnbyAllocated;
    (void)pMem;
...
/* ---- tgMM_MI__Malloc ---------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgMM_MI__Malloc( TgMEM_INFO_HEADER_P psInfo, TgVOID_PP ppReturn, TgRSIZE_C uiSize, TgRSIZE_C uiAlignment )
{
    (void)psInfo;
    (void)ppReturn;
    (void)uiSize;
...
/* ---- tgMM_MI__Free ------------------------------------------------------------------------------------------------------------------------------------------------------------ */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgMM_MI__Free( TgMEM_INFO_HEADER_P psInfo, TgVOID_PC pMem )
{
    (void)psInfo;
    (void)pMem;
    return (KTgS_OK);
...
/* ---- tgMM_MI__Realloc --------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgMM_MI__Realloc( TgMEM_INFO_HEADER_P psNew_Info, TgVOID_PP ppNew, TgMEM_INFO_HEADER_P psOld_Info, TgVOID_PC pOld, TgRSIZE_C uiNewSize, TgRSIZE_C uiNewAlignment )
{
    (void)psNew_Info;
    (void)ppNew;
    (void)psOld_Info;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
#if TgS_STAT__COMMON
static TgVOID tgMM_MI__Stats( STg2_Output_PC psOUT )
{
    (void)psOUT;
}
/*# TgS_STAT__COMMON */
