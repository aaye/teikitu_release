...
{
    return (psOUT->m_pfnWrite( psOUT, KTgMAX_RSIZE, (TgUINT_E08_CP)mbzText, FCN(Length_U08)( mbzText, nbyText ) ));
}


/* ---- FCN_IO(PrintF) ----------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRSIZE CDECL FCN_IO(PrintF)( STg2_Output_PC psOUT, CHAR(CPC) mbzText, ... )
{
    va_list                             arg_list;
    TgRSIZE                             nuiBuffer;
    TgRSIZE                             nbyBuffer;
    ...
    while (1)
    {
        mbzBuffer = (CHAR(P))TgMALLOC_NOTRACE( nbyBuffer );
