...
TgTYPE_STRUCT(STg2_EM_TE_Job_Update_PM,)
{
    ...
    TgRSIZE_AP                                  m_pxuiFree_Trigger;    /**< Example: s_xuiJob_Trigger_TE_FRM */
...
static TgRESULT                             tgEM_Job_Execute__Update( STg2_Job_CPC );
static TgRESULT                             tgEM_Job_Execute__TE( STg2_Job_CPC );
static TgRESULT                             tgEM_Job_Execute__Free_Pool( STg2_Job_CPC );
...
TgRESULT tgEM_Init_MGR( TgVOID )
{
    ...
    TgSTD_ATOMIC(store)( &s_xuiJob_Count, 0 );
...
TgRESULT tgEM_Update_MGR( TgFLOAT32_C fDT )
{
    ...
    TgSTD_ATOMIC(fetch_add)( &s_xuiJob_Count, 1 );
    if (TgFAILED(tgJM_Queue_Job( g_tiJob_Queue__OS, &s_sJob_Update )))
    {
        tgEM_Job_Execute__Update( &s_sJob_Update );
        TgSTD_ATOMIC(fetch_sub)( &s_xuiJob_Count, 1 );
    };
...
{
    return (TgSTD_ATOMIC(load)( &s_xuiJob_Count ) > 0 ? false : true);
}
...
TgVOID tgEM_Stats( STg2_Output_P psOutput )
{
    ...
    /* Yield until the previous update has completed */
    uiIndex = 0;
    while (!TgSTD_ATOMIC(compare_exchange_weak)( &s_xuiJob_Count, &uiIndex, 1))
    {
        tgTR_Yield();
        uiIndex = 0;
    };
...
static TgBOOL tgEM_Job_Execute__Submit_Event_Pool_Update( STg2_Event_Pool_Update_PCU psInit )
{
    ...
    for (TgRSIZE uiIndex = 0; uiIndex < psInit->m_uiMax; ++uiIndex)
    {
        ...
        TgSTD_ATOMIC(fetch_add)( psInit->m_pxuiFree_Trigger, 1 );
        TgSTD_ATOMIC(fetch_add)( &s_xuiJob_Count, 1 );
        ++niUpdate;

        if (TgFAILED(tgJM_Queue_Job( g_tiJob_Queue__OS, &psInit->m_apsPool[uiIndex]->m_sJob )))
        {
            psInit->m_pfnExecute( &psInit->m_apsPool[uiIndex]->m_sJob );
            TgSTD_ATOMIC(fetch_sub)( &s_xuiJob_Count, 1 );
        };
    };
...
static TgVOID tgEM_Job_Execute__Submit_Free_Pool_Fixup( STg2_Event_Pool_Update_PCU psInit )
{
    ...
    {
        while (0 != TgSTD_ATOMIC(load_explicit)( psInit->m_psFree_Job->m_pxuiTrigger, TgSTD_MEMORY_ORDER(relaxed) ));
        tgEM_Job_Execute__Free_Pool( psInit->m_psFree_Job );
        TgSTD_ATOMIC(fetch_sub)( &s_xuiJob_Count, 1 );
    };
}
...
static TgVOID tgEM_TE_Update_Internal__Waiting( STg2_EM_TE_Pool_PC psPool, STg2_EM_TE_Job_Update_PM_CPCU psParam )
{
    ...
    {
        TgERROR((TgSINT_E32 )(psTB->m_uiLocal_Index & 0xFFFF) < KTgEM_NUM_TE_IN_POOL);
        ...
        if (((TgUINT_E32)psTB->m_iStart <= (TgUINT_E32)psParam->m_iTotal) || (tgPM_FSEL_F32( psTB->m_fStart, psTB->m_fStart, KTgMAX_F32 ) <= psParam->m_fTotal))
        {
                                                ...
                                                STg2_EM_TE_Job_Update_PM_CPCU psParam )
{
    ...
    {
        TgSINT_E32                          iLocal_Index;
        TgUINT_E32                          uiIndex;

        iLocal_Index = (TgSINT_E32)(psTB->m_uiLocal_Index & 0xFFFF);
        TgERROR(iLocal_Index < KTgEM_NUM_TE_IN_POOL);

        uiIndex = (TgUINT_E32)((psTB - psPool_TB->m_asTB) & 0xFFFF);
        ...
        /* Thus, only one of these tests can ever be true (the unused value has a negative value) */
        if (((TgUINT_E32)psTB->m_iEnd <= (TgUINT_E32)psParam->m_iTotal) || (tgPM_FSEL_F32( psTB->m_fEnd, psTB->m_fEnd, KTgMAX_F32 ) <= psParam->m_fTotal))
        {
...
static TgVOID tgEM_TE_Update_Internal__Paused( STg2_EM_TE_Pool_PC psPool, STg2_EM_TE_Job_Update_PM_CPCU psParam )
{
    ...
    for (psTB = psPool->m_psWaiting; nullptr != psTB; psTB = psTB->m_psHead_Next)
    {
        ...
        psTB->m_fStart = tgPM_FSEL_F32( psTB->m_fStart, psTB->m_fStart + psParam->m_fTime_Step, psTB->m_fStart );
        psTB->m_fEnd = tgPM_FSEL_F32( psTB->m_fEnd, psTB->m_fEnd + psParam->m_fTime_Step, psTB->m_fEnd );
        psTB->m_iStart = (TgSINT_E32 )((TgUINT_E32)psTB->m_iStart & 0x80000000) | (psTB->m_iStart + 1);
        psTB->m_iEnd = (TgSINT_E32 )((TgUINT_E32)psTB->m_iEnd & 0x80000000) | (psTB->m_iEnd + 1);
    };
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgEM_Job_Execute__Update( STg2_Job_CPC psJob )
{
    union
    {
        TgUINT_E08_CP                           m_psUntyped;
        STg2_EM_TE_Job_Update_PM_P              m_psJob_Data;
    }                                   tgJob_Cast;
    TgBOOL                              bNeedFixUp;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgEM_Job_Execute__TE( STg2_Job_CPC psJob )
{
    union
    {
        TgUINT_E08_CP                       m_psUntyped;
    ...
    TgSTD_ATOMIC(fetch_sub)( psJob_Data->m_pxuiFree_Trigger, 1 );
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT tgEM_Job_Execute__Free_Pool( STg2_Job_CPC psJob )
{
    union
    {
        TgUINT_E08_CP                           m_psUntyped;
    ...
    {
        TgSINT_E32                          niUsed = ppsFP_List[uiSort0]->m_niUsed;
    ...
    if (nuiFree >= 2)
    {
        do
        {
            TgSINT_E32_C    iPool = ppsFP_List[nuiFree - 1]->m_iPool;
