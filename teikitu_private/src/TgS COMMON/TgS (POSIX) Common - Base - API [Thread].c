...
static pthread_t                            g_aTR_Thread[TgPLATFORM__THREAD_MAX];
#else
pthread_t                                   g_aTR_Thread[TgPLATFORM__THREAD_MAX];
...
static TgVOID_P tgTR_Thread_Start( TgVOID_P pParam )
{
    ...
    TgSTD_ATOMIC(store)( &g_auiTR_Thread_Stack[sParam.m_uiE32], uiCurrentStack );
