...
VAR_DECL TgUINT_E32_A                       STORAGE(niProfile_Tag);
...
TgRESULT FCN(Init)( TgVOID )
{
    ...
    TgSTD_ATOMIC(store)( &STORAGE(niProfile_Tag), 0 );
...
TgRESULT FCN(Update)( TgATTRIBUTE_MAYBE_UNUSED TgFLOAT32_C fDT )
{
    ...
    MT_CODE( tgCM_UT_LF__RW__Enter_Write_Spin_Block( &s_asProfile_Entry_RW_Lock ); )
    while (1)
    {
    ...
    uiMaxProfile = tgCM_MIN_U32( TgSTD_ATOMIC(load)( &STORAGE(niProfile_Tag) ), PROFILE_ID(KTgMAX_,) );
...
TgVOID FCN(Report_Module)( STg2_Output_PC psOUT )
{
    ...
    MT_CODE( tgCM_UT_LF__RW__Enter_Read_Spin_Block( &s_asProfile_Entry_RW_Lock ); )
...
PROFILE_ID(Tg,) FCN(Mark_Init)( TgCHAR_U8_CPC szName )
{
    ...
    uiIndex = (TgRSIZE)TgSTD_ATOMIC(fetch_add)( &STORAGE(niProfile_Tag), 1 );
    ...
    return (PROFILE_ID(tg,_Init)( STORAGE(axtiProfile_Singleton) + uiIndex, (TgUINT_E16 )uiIndex ));
}
...
PROFILE_CALL_ID(Tg,) FCN(Mark_Start)( PROFILE_ID(Tg,) tiProf )
{
    ...
    return (PROFILE_CALL_ID(tg,_Init)( &psCallRecord->m_xtiCall_Singleton, (TgUINT_E16 )((psCallRecord - STORAGE(asProfile_CallRecord)) & KTgMAX_U16) ));
}
