...
static TgUINT_E08_P                         s_pGlobal_Dict_Data;
...
TgRESULT tgSM_Init_MGR( TgVOID )
{
    ...
    {
        TgSTD_ATOMIC(store)( s_auiGlobal_Dict_Hash + uiIndex, UINT64_MAX );
    };
    TgSTD_ATOMIC(store)( &s_uiGlobal_Dict_Data_Offset, 0 );
    TgSTD_ATOMIC(store)( &s_nuiGlobal_Dict_Data, 0 );
...
/* ---- tgSM_Update_MGR ---------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgRESULT tgSM_Update_MGR( TgATTRIBUTE_MAYBE_UNUSED TgFLOAT32_C fDT )
{
...
TgVOID tgSM_Stats( STg2_Output_P psOUT )
{
    ...
    uiMax_Offset = TgSTD_ATOMIC(load)( &s_uiGlobal_Dict_Data_Offset );
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgSINT_E32 tgSM_UID_Init_String( TgCHAR_U8_CPC mbzNative_String, TgSINT_E32_C iIndex )
{
    TgDIAG( ETgMODULE_STATE__BOOTED == s_enString_MGR_State );
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgCHAR_U8_CP tgSM_UID_Query_String( TgSINT_E32_C iIndex )
{
    TgDIAG( ETgMODULE_STATE__BOOTED == s_enString_MGR_State );
...
TgSTRING_DICT_ID tgSM_Dict_Insert_String_Hash( TgCHAR_U8_CPC mbzString, TgRSIZE_C nbyMaxString, TgUINT_MAX_C uiHash )
{
    ...
    uiOffset = TgSTD_ATOMIC(load)( &s_uiGlobal_Dict_Data_Offset );
    do
    {
    ...
    while (!TgSTD_ATOMIC(compare_exchange_weak)(&s_uiGlobal_Dict_Data_Offset, &uiOffset, uiOffset + uiEntry_Size));
    ...
    tiRet = tgSTRING_DICT_ID_Init( &psEntry.ps->m_xtiString_Singleton, (TgUINT_E32)uiOffset );
    ...
    TgVERIFY(TgSTD_ATOMIC(fetch_add)( &s_nuiGlobal_Dict_Data, 1 ) < KTgSM_MAX_HASH_ENTRIES);
    TgSTD_ATOMIC(signal_fence)(TgSTD_MEMORY_ORDER(release));
    ...
    do
    {
        uiHash_Offset = TgSTD_ATOMIC(load)( s_auiGlobal_Dict_Hash + uiHash_Index );
        ...
        if (TgSTD_ATOMIC(compare_exchange_strong)(s_auiGlobal_Dict_Hash + uiHash_Index, &uiHash_Offset, uiOffset))
        {
            return (tiRet);
        };
...
TgSTRING_DICT_ID tgSM_Dict_Find_Id_By_Hash( TgCHAR_U8_CPC mbzString, TgUINT_MAX_C uiHash )
{
    ...
    do
    {
        uiOffset = TgSTD_ATOMIC(load)( s_auiGlobal_Dict_Hash + uiHash_Index );
...
TgSTRING_TABLE_ID tgSM_Table_Load( STg2_Input_PC psInput, TgRSIZE_C uiOffset_Start )
{
    ...
    for (TgUINT_E32 uiIndex = 1; uiIndex < KTgSM_NUM_STRING_TABLE_MAX; ++uiIndex)
    {
        TgUN_PTR                            sPTR;
        ...
        if (!TgSTD_ATOMIC(compare_exchange_strong)( &s_axtiString_Table_Singleton[uiIndex].m_uiKI, &tiRet.m_uiKI, KTgMAX_U32))
        {
        ...
        TgCRITICAL_MSGF( nullptr != s_apsString_Table[uiIndex], STD_MSG_LITERAL_1, STD_MSG_POST, u8"Failed to allocate memory." );
