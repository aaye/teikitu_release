...
/*  »Project«   Teikitu Gaming System (TgS) (∂)
    »File«      TgS (WINDOWS) Common - Base - API [Thread].c
...
__declspec(dllimport) HINSTANCE __stdcall
LoadPackagedLibrary(
    TgCHAR_WC_CP ARG0, TgUINT_E32 ARG1 );
...
__declspec(dllimport) TgSINT_E32 __stdcall
FreeLibrary(
    HINSTANCE ARG0 );
...
static DWORD                                s_auiTR_Thread_OS_Id[TgPLATFORM__THREAD_MAX];

#if !TgCOMPILE__THREAD_INFO
static TgVOID_P                             g_aTR_Thread[TgPLATFORM__THREAD_MAX];
#else
TgVOID_P                                    g_aTR_Thread[TgPLATFORM__THREAD_MAX];
...
TgTHREAD_ID tgTR_Register_Main_Thread( TgCHAR_U8_CPC szName )
{
    ...
    for (uiIndex = 0; uiIndex < TgPLATFORM__THREAD_MAX; ++uiIndex)
    {
        ...
        TgSTD_ATOMIC(store)( &g_auiTR_Thread_Stack[uiIndex], tgTR_Stack_Size() );
        ...
        tiThread = tgTHREAD_ID_Init(g_axtiTR_Thread_Singleton + uiIndex, (TgUINT_E32)uiIndex);
...
/* ---- tgTR_Create -------------------------------------------------------------------------------------------------------------------------------------------------------------- */
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
TgTHREAD_ID tgTR_Create( TgPLATFORM_THREAD_FCN pfnThread, TgUINT_PTR_C uiParam, TgUINT_E32_C uiStack, ETgTHREAD_PRIORITY_C enPriority, TgCHAR_U8_CPC szName )
{
    TgRSIZE                             uiIndex;
    TgTHREAD_ID                         tiThread;
    ...
    for (uiIndex = 0; uiIndex < TgPLATFORM__THREAD_MAX; ++uiIndex)
    {
        ...
        TgSTD_ATOMIC(store)( &g_auiTR_Thread_Stack[uiIndex], KTgMAX_RSIZE-1 );
...
TgVOID tgTR_Close( TgTHREAD_ID_C tiThread )
{
    ...
    TgPARAM_CHECK((tiThread.m_uiKI != KTgID__INVALID_VALUE) && (tiThread.m_uiI < TgPLATFORM__THREAD_MAX));
    ...
    TgSTD_ATOMIC(store)( &g_auiTR_Thread_Stack[tiThread.m_uiI], KTgMAX_RSIZE );
...
ETgTHREAD_STATUS tgTR_Status( TgTHREAD_ID tiThread )
{
    ...
    if (tiThread.m_uiKI == KTgID__INVALID_VALUE || tiThread.m_uiI >= TgPLATFORM__THREAD_MAX)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            uiStack = TgSTD_ATOMIC(load)( &g_auiTR_Thread_Stack[tiThread.m_uiI] );
...
TgRSIZE tgTR_Stack_Size( TgVOID )
{
    ...
    return ((TgUINT_E32)((TgUINT_E08_P)(pTEB->StackBase) - (TgUINT_E08_P)(pTEB->StackLimit)));
}
...
TgVOID tgTR_Print_Threads( STg2_Output_P pOutput )
{
    ...
    do
    {
        if (te32.th32OwnerProcessID == uiOwnerPID)
        {
            ...
            for (iIndex = 0; iIndex < TgPLATFORM__THREAD_MAX; ++iIndex)
            {
            ...
            if (iIndex >= TgPLATFORM__THREAD_MAX)
            {
            ...
            else
            {
                ...
                uiStack = TgSTD_ATOMIC(load)(&g_auiTR_Thread_Stack[iIndex]);
...
static TgVOID tgTR_Thread_Set_Name( TgRSIZE_C iIndex )
{
    ...
    g_pfnSleep(10);
    TgERROR((TgUINT_E32)s_auiTR_Thread_OS_Id[iIndex] == g_pfnGetCurrentThreadId()); /* OS race condition for the win - sometimes not flushed */
...
static unsigned long __stdcall tgTR_Thread_Start( void* pParam )
{
    ...
    TgSTD_ATOMIC(store)( &g_auiTR_Thread_Stack[sParam.m_uiE32], tgTR_Stack_Size() );
