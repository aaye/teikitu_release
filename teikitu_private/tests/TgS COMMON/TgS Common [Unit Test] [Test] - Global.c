...
TEST_METHOD( UNIT_TEST__TEST__GB_CMD_Set_Command_Line )
{
    TgCHAR_U8                           szCmdLine_Original[KTgMAX_CMD_LINE_SIZE];
    TgCHAR_U8                           szCmdLine_Compare[KTgMAX_CMD_LINE_SIZE];
    TgCHAR_U8                           szCmdLine[KTgMAX_CMD_LINE_SIZE];
    TgSINT_E32                          iIter;
    TgSINT_E08                          iRet;

    szCmdLine_Original[0] = 0;
    for (iIter = 0; iIter < tgGB_CMD_Query_Argument_Count();)
    {
        tgUSZ_AppendF( szCmdLine_Original, TgARRAY_COUNT( szCmdLine_Original ), tgGB_CMD_Query_Argument( iIter ), KTgMAX_RSIZE );
        if (++iIter >= tgGB_CMD_Query_Argument_Count()) break;
        tgUSZ_AppendF( szCmdLine_Original, TgARRAY_COUNT( szCmdLine_Original ), u8" ", KTgMAX_RSIZE );
    }
    ...
    szCmdLine_Compare[0] = 0;
    for (iIter = 0;;)
    {
        tgUSZ_AppendF( szCmdLine_Compare, TgARRAY_COUNT( szCmdLine_Compare ), tgGB_CMD_Query_Argument( iIter ), KTgMAX_RSIZE );
        if (++iIter >= tgGB_CMD_Query_Argument_Count()) break;
        tgUSZ_AppendF( szCmdLine_Compare, TgARRAY_COUNT( szCmdLine_Compare ), u8" ", KTgMAX_RSIZE );
    }

    Test__Expect_EQ(true, TgSUCCEEDED(tgUSZ_Compare( &iRet, szCmdLine_Compare, TgARRAY_COUNT( szCmdLine_Compare ), 0, szCmdLine, TgARRAY_COUNT( szCmdLine ), 0 )) && (0 == iRet));

    tgGB_CMD_Set_Command_Line( szCmdLine_Original );
...
TEST_METHOD( UNIT_TEST__TEST__GB_CMD_Query_Argument )
{
    ...
    Test__Expect_EQ(true, TgSUCCEEDED(tgUSZ_Compare( &iRet, tgGB_CMD_Query_Argument( 0 ), KTgMAX_RSIZE, 0, u8"test.exe", KTgMAX_RSIZE, 0 )) && (0 == iRet));
...
TEST_METHOD( UNIT_TEST__TEST__GB_CMD_Query_Argument_Index )
{
    ...
    Test__Expect_NE(0, tgGB_CMD_Query_Argument_Index(u8"test"));
    Test__Expect_EQ(0, tgGB_CMD_Query_Argument_Index(u8"test.exe"));
