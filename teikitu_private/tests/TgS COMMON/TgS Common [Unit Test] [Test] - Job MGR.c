...
static TgRESULT                             Job_Unit_Test__JM_Queue_Job( STg2_Job_CPC psJob );
static TgRESULT                             Job_Unit_Test__JM_Query_Thread_Id( STg2_Job_CPC psJob );

static TgSINT_E32_A                         s_iTest;
...
/*# TgCOMPILE__THREAD */
    ...
    while (KTgJOB_TEST_MAX != TgSTD_ATOMIC(load)( &s_iTest ))
    {
        Unit_Test__Sleep( 1000000 );
    };
...
TEST_METHOD( UNIT_TEST__TEST__JM_Query_Thread_Id )
{
    ...
    if (TgFAILED(tgJM_Queue_Job( g_tiJob_Queue__OS, &sJob_Test )))
    {
        Job_Unit_Test__JM_Query_Thread_Id( &sJob_Test );
        bMT_Enabled = false;
    }

    while (0 == TgSTD_ATOMIC(load)( &s_iTest ))
    {
        Unit_Test__Sleep( 1000000 );
    };
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
static TgRESULT Job_Unit_Test__JM_Queue_Job( TgATTRIBUTE_MAYBE_UNUSED STg2_Job_CPC psJob )
{
    TgSTD_ATOMIC(fetch_add)( &s_iTest, 1 );
...
static TgRESULT Job_Unit_Test__JM_Query_Thread_Id( STg2_Job_CPC psJob )
{
    ...
    union
    {
        UNIT_TEST_JOB_DATA_CP               psJob_Data;
        TgUINT_E08_CP                       puiE08;
    }                                   uJob_Data;

    iIndex = TgSTD_ATOMIC(fetch_add)( &s_iTest, 1 );
