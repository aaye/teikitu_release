...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
#if defined(TgBUILD_FEATURE__GRAPHICS) && !defined(TgFEATURE_GRAPHICS__REF)
static TgRESULT UNIT_TEST__TEST__FX_GPU_Render_Simple_Job( STg2_Job_PC psJob )
{
...
TEST_METHOD( UNIT_TEST__TEST__FX_GPU_Render_Simple )
{
    ...
    for (s_uiFrame_Counter = 0; !bAll_Windows_Closed; ++s_uiFrame_Counter)
    {
        ...
        if (!bAll_Windows_Closed)
        {
            ...
            atomic_store( &sSE_Context.m_xnuiFrame_Mesh, 0 );
            atomic_store( sSE_Context.m_asView[0].m_axnuiFrame_Mesh_Inst + ETgSE_DRAW_TYPE__OPAQUE, 0 );
            atomic_store( sSE_Context.m_asView[0].m_axnuiFrame_Mesh_Inst + ETgSE_DRAW_TYPE__TRANSPARENT, 0 );

            atomic_store( &g_sRN_Output.m_xnuiFX_Header, 0 );
            g_sRN_Output.m_nbyUsed = 0;
            g_sRN_Output.m_nbyFree = KTgRN_MAX_PRTCL_BUFFER_SIZE;
...
/* ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- */
#if defined(TgBUILD_FEATURE__GRAPHICS) && !defined(TgFEATURE_GRAPHICS__REF)
static TgBOOL tgUT_IN_Gamepad_Camera( TgUINT_PTR_C UNUSED_PARAM ARG0, TgFLOAT32_C UNUSED_PARAM ARG1, STg2_IN_Event_CPC psIN_Event )
{
